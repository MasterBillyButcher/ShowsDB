...
function activateAdmin() {
  ...
  /* Symmetric with deactivateAdmin() — refresh views so admin sees
     hidden shows/contestants immediately without a page reload. */
...
function deactivateAdmin() {
  ...
  /* Re-render everything that reads admin-active status. Without this,
     hidden shows/contestants that were visible while logged in as admin
     stay visible in the sidebar, Overview cards, and Rankings until a
     full page reload — a real data leak to public visitors. */
  if (typeof rebuildSidebar     === 'function') rebuildSidebar();
  if (typeof renderOverview     === 'function') renderOverview();
  if (typeof updateStats        === 'function') updateStats();
  if (typeof renderRankings     === 'function') renderRankings();
